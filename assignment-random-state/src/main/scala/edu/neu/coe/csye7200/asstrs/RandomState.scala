...
// TO BE IMPLEMENTED
    // the idea is to use manually do the flatmap, map first, then do the flatten
    map(f).get
  }
...
case class JavaRandomState[T](n: Long, g: Long => T) extends RandomState[T] {
  ...
  def next: RandomState[T] = {
// TO BE IMPLEMENTED 
    val seed = new Random(n).nextLong()
    JavaRandomState(seed, g)
  }

  /*END*/
  // Hint: Think of the input and output.
  // 5 points
  // TO BE IMPLEMENTED g(n)
  def get: T = g(n)
...
object RandomState {
  ...
  // 4 points
  val longToDouble: Long => Double = n => n.toDouble / Long.MaxValue.toDouble
// TO BE IMPLEMENTED 
